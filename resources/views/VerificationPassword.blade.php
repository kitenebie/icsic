...
<body class="bg-gray-50 min-h-screen flex items-center justify-center p-6">
    <div class="w-full max-w-md bg-white rounded-2xl shadow-lg p-6">
        ...
        <form id="pwForm" class="space-y-4" novalidate>
            ...
            <div>
                ...
                <div class="relative">
                    <input id="password" type="password"
                        class="pw-input border border-gray-200 rounded-lg px-4 py-2 pr-12 focus:outline-none focus:ring-2 focus:ring-emerald-300"
                        autocomplete="new-password" required />
                    <button type="button" id="togglePw"
                        class="absolute right-2 top-1/2 -translate-y-1/2 text-sm text-gray-600 hover:text-gray-800">Show</button>
                </div>
            </div>
            ...
            <div class="pt-2">
                <button type="submit"
                    class="w-full px-4 py-2 bg-emerald-500 text-white rounded-lg shadow hover:brightness-95">Set
                    password</button>
            </div>
        </form>
    </div>
    ...
    form.addEventListener('submit', (e) => {
        ...
        if (problems.length > 0) {
            Swal.fire({
                ...
                html: '<ul style="text-align:left;">' + problems.map(p => `<li>• ${p}</li>`).join('') +
                    '</ul>'
            });
            return;
        }
