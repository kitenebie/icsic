<div class="comments-modal {{ $this->closeCommentModal ? 'hidden' : 'flex' }}" 
    ...
    <style>
    /* Facebook-inspired Comments Component Styles */
    .comments-modal {
        position: fixed;
        inset: 0;
        z-index: 1000;
    ...
    @media (prefers-color-scheme: dark) {
        .comments-container, .comment-bubble, .comment-input-container {
            background-color: #242526;
        }
        .comments-list {
            background-color: #18191a;
        }
        .comments-title, .comment-author, .comment-text {
            color: #e4e6ea;
        }
        .comment-actions, .reply-button, .placeholder-text {
            color: #b0b3b8;
        }
        .comment-input {
            color: #e4e6ea;
